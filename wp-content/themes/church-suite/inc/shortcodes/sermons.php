...
<?php
	...
	while ($query -> have_posts()) : $query -> the_post();
		...
		$sermon_video = (array_key_exists('sermon_video',$w_sermon_meta))?'<a href="'.$w_sermon_meta['sermon_video'].'" class="videolb '.$button.'" target="_self"><i class="fa-play-circle"></i><span class="media_label">'.esc_html__('WATCH','webnus_framework').'</span></a>':'';
		...
		if ($type=='toggle'){	
			...
			echo '<article><a href="'.$permalink.'"><i class="sermon-icon '.$icon.'"></i><h4>'.$title.'</h4></a><div class="sermon-detail">'. $desc.'</div><div class="media-links">'. $sermon_audio . $sermon_video . $sermon_pdf . $sermon_read. '</div></article>';  
		}else if ($type=='grid'){
			...
			echo '<div class="col-md-2 col-sm-2"><article>'
				.$image_media.
			...
			echo ($rcount == 1)?'<div class="row">':'';
			$image_media=($image)?'<figure class="sermon-img">'.$image.'</figure>':'';			
			echo '<div class="col-md-4 col-sm-4"><article>'
				.$image_media.
				'<h4><a href="'.$permalink.'">'.$title.'</a></h4>
	...
	endwhile;
	echo(($type=='grid')&&($rcount % 2==0))?'</div>':'';
	echo((($type=='simple')OR($type=='clean'))&&($rcount !=1))?'</div>':'';
	echo "</div>";
