 ...
 * Appendipity Themes
 *
 * @package Appendipity\Widgets
 * @author  Appendipity
 */
add_action( 'widgets_init', 'app_featured_load_widgets' );
 ...
 * @since 0.1
 */
function app_featured_load_widgets() {
	register_widget( 'Appendipity_Featured_Post' );
}
/**
 * Appendipity Featured Post widget class.
 *
 * @since 0.1.8
 *
 * @package Appendipity\Widgets
 */
class Appendipity_Featured_Post extends WP_Widget {
	...
	function __construct() {
		...
		$this->defaults = array(
			...
			'content_limit'           => '',
			'more_text'               => __( 'Show Notes', 'genesis' ),
			'extra_num'               => '',
			'extra_title'             => '',
			'more_from_category'      => '',
		...
		parent::__construct( 'featured-post', __( 'Appendipity - Featured Posts', 'genesis' ), $widget_ops, $control_ops );
	...
	function widget( $args, $instance ) {
		...
		global $wp_query, $_genesis_displayed_ids, $app_options;
		...
		if ( have_posts() ) : while ( have_posts() ) : the_post();
			...
			if ( ! empty( $instance['show_content'] ) ) {
				...
				$showpwd_fp 		= $app_options[ 'showpwd' ];
				...
				if ( $podcast ) {
					...
					echo '<div class="podcast-entry' . ( ( empty( $instance['show_title'] ) ) ? ' no-title' : '' ) . '">';
						echo '<div class="player">';
							...
							if ( ( $showpwd_fp == 1 ) && ( $show_spp_player != 1 ) ) {
								echo '<div class="dnld-play">';
									echo '<a class="play" target="_blank" href="' . $podcast . '" title="Play in New Window"><i class="icon-export"></i></a>';
									echo '<a rel="nofollow" download="' . $p_file . '" href="' . $podcast . '" title="Download"><i class="icon-download"></i></a>';
								echo '</div>';
							}
						echo '</div>';
					echo '</div>';
					...
					echo '<div class="podcast-entry' . ( ( empty( $instance['show_title'] ) ) ? ' no-title' : '' ) . '">';
						echo '<div class="player">';
							...
							if ( ( $showpwd_fp == 1 ) && ( $filetype['type'] != 'video/mp4' ) && ( $show_spp_player != 1 ) ) {
								echo '<div class="dnld-play">';
									echo '<a class="play" target="_blank" href="' . $ppdata['url'] . '" title="Play in New Window"><i class="icon-export"></i></a>';
									echo '<a rel="nofollow" download="' . basename( $ppdata['url'] ) . '" href="' . $ppdata['url'] . '" title="Download"><i class="icon-download"></i></a>';
								echo '</div>';
							}
						echo '</div>';
					echo '</div>';
						...
						echo '<div class="player">';
							...
							if ( ( $showpwd_fp == 1 ) && ( $show_spp_player != 1 ) ) {
								echo '<div class="dnld-play">';
									...
									echo '<a rel="nofollow" download="' . basename( $spp_podcast ) . '" href="' . $spp_podcast . '" title="Download"><i class="icon-download"></i></a>';
								echo '</div>';
							}
						echo '</div>';
					echo '</div>';
